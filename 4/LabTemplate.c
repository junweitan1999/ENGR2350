...
float ksteer = 553;                   // Proportional Gain constant for steering
float ksteer1 = 90;
...
void main(void){
    ...
    while(1){
        ...
        if (new_print){
            // Print import stuff
            printf("%d %d %d %d \r\n",heading, range, SERVO_PW, MOTOR_PW);
            new_print = 0;
        }

        heading_error = heading - 225; //calculation of heading error
        // the following code is to make sure the heading error is always between 180 and -180
        ...
        // use proportioonal control to adjust the servopw making the car driving toward obstacle
        SERVO_PW = SERVO_CENTER - ksteer*(heading_error)/ksteer1;
        PCA0CP0 = 0xFFFF - SERVO_PW;


        if (range >120){
            range = 120;  // if range is greater than 120 then just assume range is 120 since motor will go fully forward when range = 120
        }

        range_error = range - 60;

        MOTOR_PW = MOTOR_NEUT + kdrive*(range_error)/kdrive1;
        PCA0CP2 = 0xFFFF - MOTOR_PW;
        //approach obstacle 60cm working
        ...
        if (!PB && flag){
            ...
            // after make the wireless actuator stop making the car driving towards the goal
            counts = 0;
            while(counts <100){
                Sim_Update();
                SERVO_PW = SERVO_LEFT;
...
void start(void){  // start the wireless actuator
    unsigned char addr = 0x42;          // the address of the ranger is 0xE0
    unsigned char Data[1];
    Data[0] = 0x01;
    i2c_write_data(addr,1,Data,1);
    flag=1;
}
